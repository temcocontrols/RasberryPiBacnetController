 ...
 * the base class.  Most of these are dummy functions that only return
 * a warning message to the calling routine.  A well defined derived
...
//TBD: Remove '#define SERIAL_COMM'. Define this macro in Makefile
#define SERIAL_COMM
...
//	}
    ...
    for (;;)
	{
		...
		if ((milliseconds != FOREVER) && ((ReadTime() - start_time) >= milliseconds))
		{
			break;
		}
 ...
 * 
 * DESCRIPTION:
 * This non-virtual member function of class RS232 operates by repeatedly 
 * calling the virtual function write_byte() for the derived class.  The 
 * milliseconds parameter defines how long the function will keep trying 
 * before giving up. While waiting, the idle function is called.
 *****************************************************************************/
...
//	   return error_status;
	...
	for (;;)
	{
		//TBD: Create a wrapper over serialPutchar() to return 'write_status'
		//write_status = serialPutchar(serialFd, c);
 ...
 * 
 * DESCRIPTION:
 * This non-virtual member function of class RS232 writes out a buffer using 
 * the virtual write_buffer() routine.  It has two additional parameters beyond
 * those used by write_buffer(), which are a timeout value and a terminator.  
 * The terminator can be used to automatically append a CR/LF pair to output
 * strings.
 *****************************************************************************/
					 ...
					 char *terminator )
{
	...
	for (;;)
	{
		...
		*/
		...
		//TBD: Please into below if() condition
 ...
 * 
 * DESCRIPTION:
 * There are two versions of the non-virtual ReadBuffer() function defined 
 * for the base class.  They differ only in what causes their normal 
 * termination.  This version terminates only when it reads in a full buffer 
 * of data, or times out.  The next version stops when it sees the 
 * terminator string specified as a parameter.
 *****************************************************************************/
 ...
 * 
 * DESCRIPTION:
 * This version of ReadBuffer() looks for a termination string in the 
 * incoming data stream.  Because of this, it has to read in characters one 
 * at a time instead of in blocks.  It looks for the terminator by doing a 
 * strncmp() after every new character is read in, which is probably not the 
...
int RS232::Break( long duration )
{
    UNUSED( duration );
    return RS232_FUNCTION_NOT_SUPPORTED;
}

int RS232::SoftwareOverrunError( int clear )
{
    UNUSED( clear );
    return RS232_FUNCTION_NOT_SUPPORTED;
}
...
int RS232::XonXoffHandshaking( int enable )
{
    UNUSED( enable );
    return RS232_FUNCTION_NOT_SUPPORTED;
}

int RS232::RtsCtsHandshaking( int enable )
{
    UNUSED( enable );
    return RS232_FUNCTION_NOT_SUPPORTED;
}

int RS232::DtrDsrHandshaking( int enable )
{
    UNUSED( enable );
    return RS232_FUNCTION_NOT_SUPPORTED;
}

int RS232::Dtr( int setting )
{
    UNUSED( setting );
    return RS232_FUNCTION_NOT_SUPPORTED;
}

int RS232::Rts( int setting )
{
    UNUSED( setting );
    return RS232_FUNCTION_NOT_SUPPORTED;
}

int RS232::Peek( void *buffer, unsigned int count )
{
    UNUSED( buffer );
    UNUSED( count );
    return RS232_FUNCTION_NOT_SUPPORTED;
