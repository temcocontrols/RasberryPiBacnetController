...
//			outtextxy(viewports[0].top.x-(i+2)*8,viewports[0].bot.y-16,"Û");
		}
 }
// clear_semaphore(&screen);
 v=1;
...
//		outtextxy(viewports[0].top.x-(v*56)-(j+2)*8,viewports[0].bot.y-16,"Û");
	 }
	 clear_semaphore(&screen);
	 v++;
	}
...
// set_semaphore(&screen);
						...
						y3 = dyv-y - 4;
						setcolor(Black);
						outtextxy(x2+4, dyv - (*ppix_table)-4, "Û");
						setcolor(color[ind]);
						moveto(x,dyv-y);
