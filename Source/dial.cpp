...
//				 }
	  ...
	  if (DIAL_checkconnection)
	  {
		...
		if( DIAL_updategrp==1 )
		{
		 ...
		 {
			update_length = 0;
			if( net_call(19, current_point_GRP, updatebuffer, &update_length, panel_GRP, network_GRP, NETCALL_CHECKDUPLICATE|NETCALL_NOTTIMEOUT,17)==SUCCESS )
			{
			 if(DIAL_updategrp==1)  // the buffer still exist?
			 {
				if(group_element_temp)  // the buffer still exist?
				{
...
//			biosprint(0, '\f', 0);
		...
		if (save_file)
		{
		 ...
		 ptr_panel->savefile(saveloadfilename, &Panel_Info1, ptr_panel->table_bank, 1);
		 continue;
		}
		if (load_file)
		{
		 load_file = 0;
		 if(main_screen)
			 ptr_panel->loadprg(saveloadfilename,&Panel_Info1,ptr_panel->table_bank, 1);
		 continue;
		}
		if (updatenet_flag)
		{
