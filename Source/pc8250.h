...
//	   virtual int read_byte( void );
	 ...
	 public :
		virtual int write_buffer( char *buffer,
							 unsigned int count = -1, int type = 0 );
		virtual int read_byte( void );
      void resettxrunning(void){isr_data->tx_running=0;}

		PC8250( RS232PortName port_name,
