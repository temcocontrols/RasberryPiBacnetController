 ...
 * 
 * DESCRIPTION:
 * read_buffer() pulls in only as many bytes as are immediately available. 
 * Any high-level functions such as timing out or looking for a terminator 
 * are handled by one of the higher level Read() routines from class RS232.
 *****************************************************************************/
...
int PC8250::read_buffer( char *buffer, unsigned int count )
{
	...
	while (serialDataAvail (serialFd) > 0)
	{
		...
		*buffer++ = (char)serialGetchar(serialFd);
		count--;
	}
...
int PC8250::write_buffer( char *buffer, unsigned int count, int type )
{
	...
	for (;;)
	{
		...
		if (type)
		{
			//TBD: Whats the meaning of literal 0x10, 0x11, 0x13
			if ((*buffer==0x10) || (*buffer==0x11) || (*buffer==0x13))
			{
				//if ( !isr_data->TXQueue.Insert( 0x10 ) )
				//	break;
				//TBD: Wrapper over serialPutchar() to return status as above
				serialPutchar (serialFd, 0x10);
				
				//if ( !isr_data->TXQueue.Insert( *buffer|0x90 ) )
				//	break;
				//TBD: Wrapper over serialPutchar() to return status as above
				serialPutchar (serialFd, (*buffer | 0x90));
